...
# - > do not need total
    ...
    df_agg['Video pub­lish time'] = pd.to_datetime(df_agg['Video pub­lish time']) # pandas datetime values helps us with sorting, day difference
...
# Figure out what was the date 12 months before the recent videos was published
metrics_data_12mon = df_agg_diff['Video pub­lish time'].max() - pd.DateOffset(months=12)
median_agg = df_agg_diff[df_agg_diff['Video pub­lish time'] >= metrics_data_12mon].median()
...
#get last 12 month of data rather than all data
...
views_days = pd.pivot_table(df_time_diff_yr, index ='days_published', values='Views', aggfunc=[np.mean,np.median,q80,q20]).reset_index()
...
if add_sidebar == 'Aggregate Metrics':
    df_agg_metrics = df_agg[['Video pub­lish time','Views','Likes','Sub­scribers','Shares','Com­ments ad­ded','RPM(USD)','Av­er­age per­cent­age viewed (%)',
                            'Avg_duration_sec','Engagement_ratio','Views / sub gained']]
    metrics_date_6mo = df_agg_metrics['Video pub­lish time'].max() - pd.DateOffset(months=6)
    metrics_date_12mo = df_agg_metrics['Video pub­lish time'].max() - pd.DateOffset(months =12)
    metric_medians6mo = df_agg_metrics[df_agg_metrics['Video pub­lish time'] >= metrics_date_6mo].median()
    metric_medians12mo = df_agg_metrics[df_agg_metrics['Video pub­lish time'] >= metrics_date_12mo].median()
    ...
    df_agg_diff_final = df_agg_diff.loc[:,['Video title','Publish_date','Views','Likes','Sub­scribers','Shares',
                                           'Com­ments ad­ded','RPM(USD)','Av­er­age per­cent­age viewed (%)',
                                           'Avg_duration_sec', 'Engagement_ratio','Views / sub gained']]
...
if add_sidebar == 'Individual Video Analysis':
    ...
    fig2.add_trace(go.Scatter(x=views_cumulative['days_published'], y=views_cumulative['median_views'], mode='lines',
